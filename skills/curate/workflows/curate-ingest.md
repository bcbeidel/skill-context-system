...
```bash
python ${CLAUDE_PLUGIN_ROOT}/skills/curate/scripts/propose.py --kb-root <kb_root> --topic "<topic_name>" --relevance "<relevance>" --proposed-by "ingest" --rationale "Ingested from: <url>"
```
...
Show the user:
- `knowledge/_proposals/<slug>.md` -- Proposal with source URL pre-filled
